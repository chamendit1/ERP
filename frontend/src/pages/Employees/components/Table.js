...
const EnhancedTableToolbar = (props) => {
  ...
  return (
    <Toolbar sx={{ pl: { sm: 2 }, pr: { xs: 1, sm: 1 }}}>
      {(
        <Typography sx={{ flex: '1 1 100%', fontWeight: 'bold' }}>
          Customers
        </Typography>
      )}
      {
              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Table */}
                      <TableCell  align="left" onClick={() => openClient(row._id)}>{row.id}</TableCell>
                      <TableCell  align="left" onClick={() => openClient(row._id)}>{row.firstName}</TableCell>
                      <TableCell  align="left" onClick={() => openClient(row._id)}>{row.lastName}</TableCell>
                      <TableCell  align="left" onClick={() => openClient(row._id)}>{row.position}</TableCell>
